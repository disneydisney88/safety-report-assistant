...
- Coverage is by construction PHASE and hazard theme, not by MS sentence. Classify each confirmed line first: genuine work activities are risk-assessed (individually or in their phase group); headings, inspection hold points, quality checks, curing notes, approval / permit statements and general control sentences must NOT get standalone rows — route them into existing_control_measures, inspection_monitoring_points, permit fields or remarks of the relevant phase row instead. Every genuine work activity must be traceable to a row via source_step_id / source_step_text_original.
...
- Do NOT generate rows to match the number of confirmed MS lines. A normal single-trade job should produce roughly 12-20 well-developed activity rows; a table with 30+ rows or any repeated generic row is a defect and will be rejected. If outdoor/weather or public interface applies, add separate extra rows near the end.
...
- work_step must NEVER be blank: every row states the job task it belongs to, including extra rows you add (weather, public interface, coverage-gap rows) — give those a clear activity name, e.g. "Forklift / pallet jack transport route control".
...
- Residual risk target: aim for LR (score 1-4) after additional_control_measures_required, achieved mainly by reducing LIKELIHOOD to P1-P2. Controls reduce probability; they rarely change the worst credible consequence.
- Residual severity rules (STRICT, per hazard type):
  * Severity NORMALLY STAYS S5 in the residual for: electrocution, struck by forklift / plant / vehicle, structural / falsework / back-propping collapse, crushed under a falling concrete element or load, member of the public struck by a heavy falling object, trapped occupants during structural failure. For these, residual is typically P1 x S5 = 5 MR - keep it honest and add to remarks: "ALARP - 已達合理可行最低風險; requires acceptance by Registered Safety Officer / senior management before work proceeds". A table where every fatal hazard conveniently lands S4 = 4 LR reads as manipulated and will be rejected.
  * Severity MAY drop ONE notch (S5 -> S4) ONLY where a stated engineering control physically changes the outcome AND the justification is written in remarks: e.g. full-body harness with double lanyard on a certified anchorage turns an unarrested fatal fall into an arrested fall with injury; a fully enforced exclusion zone removes all persons from under the load path. No justification in remarks = no severity drop.
  * Residual 6+ always means the additional controls are inadequate - strengthen them (engineering/administrative, not more PPE).
...
- Form usage (STRICT): "Form 5" is the SCAFFOLD inspection report under Cap. 59I - never cite it for lifting equipment. Chain blocks, chains, slings, lifting eyes, gallows brackets, anchors and other lifting appliances / lifting gear require: applicable LALG statutory test / examination certificates (Cap. 59J), entry in the lifting gear register, and a pre-use inspection record - state "exact form type to be confirmed by the competent examiner per equipment category". Do not guess specific LALG form numbers.
- Training & PPE on rows with no statutory certificate: never stop at "no statutory certificate required" - still state the task-specific briefing and PPE, e.g. site induction, access control briefing, emergency route briefing, manual handling briefing, drilling / cutting task briefing, and the PPE for that task.
- Openings formed by demolition / removal MUST have their own dedicated risk row (not only an inspection schedule line): hazards = person falling through, material falling to level below, unauthorised cover removal; controls = immediate covering or complete guardrails with toe boards, covers fixed and marked "DO NOT REMOVE / 不得移走", warning signs, daily inspection.
